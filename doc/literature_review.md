...
|---|---|---|
| 固定机队在 SGP 中循环 | 40 架飞机、A/B 波次循环 | 保留封闭机队语义 |
...
已在独立分支 `experiment/yoon-sgp-baseline` 启动 B0.1：
...
6. 明确拒绝在 B0.2 完成前把 `paper_yoon_2023` 当作可执行环境。

验证结果：

- 项目 Conda 环境下 17 项单元/集成测试全部通过；
...
下一步 B0.2 将实现配置驱动的 SGP、FlyPro 任务事件、位置/路径占用、机库、
升降机和计划维修，再运行论文 8 个案例的 1000 次重复实验。

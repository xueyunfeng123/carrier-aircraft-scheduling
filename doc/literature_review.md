...
- **迁移判断 \[本项目推断]**：滚动 CP-SAT 和 RL 可统一理解为不同强度的闭环近似；这有利于在论文中解释各求解器，而不是把它们写成互不相关的方法集合。
...
- 仿真预算较高时，可用代理模型或 ADP 做候选筛选，但最终性能必须回到同一个真实 DES 上评估。
...
4. **统一比较**：在同一 DES、同一随机种子集合和同一时间预算下比较 Random、FIFO、SPT、EDD、Heuristic、CP-SAT 与 RL。
...
- Random、FIFO、SPT、EDD、Heuristic、CP-SAT、RL 统一接口。
...
- `seed=10007`、60 分钟、12 波次基准与主分支一致：Random 141、FIFO 143、SPT 140、EDD 143、Heuristic 152、CP-SAT 140；
...
CP-SAT 分别完成 143/152/157/158/157/162 架次。该结果
...
Random/FIFO/SPT/EDD/Heuristic/CP-SAT 完成架次为
66/74/90/53/93/86；关闭空间图的严格匹配对照为
143/152/157/158/157/162。吞吐量下降幅度说明当前原子路径预留较为
